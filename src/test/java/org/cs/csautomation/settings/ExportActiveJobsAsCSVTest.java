 ...
 * export the active job as CSV by clicking on right click option and mass
 * option and then verify the exported CSV File.
 * 
 * @author CSAutomation Team
 */
public class ExportActiveJobsAsCSVTest extends AbstractTest {

    private WebDriverWait  waitForReload;
    private CSPortalHeader csPortalHeader;
    private FrameLocators  locator;
    private ActiveJobsPage activeJobsPage;
    private String         activeJobExportAsCSVSheetName = "ActiveObjectExportAsCSV";
     ...
     * and then Export Active Job As CSV by right click and by mass option and
     * the verify exported file.
     * 
     * @param activeJobLabelName String object containing Job Name.
     */
    @Test(dataProvider = "insertLabelTestData")
    public void testExportActiveJobAsCSV(String activeJobLabelName) {
        try {
            ...
            exportCsvByRightClick(waitForReload);
            exportCsvWithMassAction(waitForReload);
        } catch (Exception e) {
            CSLogger.debug("Failed To Export Job By CSV", e);
            Assert.fail("Failed To Export Job By CSV");
        }
    }
     ...
     * and Export as CSV option.
     * 
     * @param waitForReload WebDriverWait Object
     */
    private void exportCsvByRightClick(WebDriverWait waitForReload) {
        ...
        CSLogger.info("Right click to select Export as CSV Option");
        CSUtility.tempMethodForThreadSleep(2000);
        browserDriver.switchTo().defaultContent();
        browserDriver.switchTo().frame(locator.getFrmCsPopupDivFrame());
        activeJobsPage.exportAsCSV().click();
        CSLogger.info(
                "Clicked on option 'Export as CSV' displayed after clicking"
                        + "on right click option");
        CSUtility.getAlertBox(waitForReload, browserDriver).accept();
        CSUtility.tempMethodForThreadSleep(2000);
        popupWindowAfterExportAndVerifyExportedCSV(runedJobs);
        CSLogger.info(
     ...
     * @param waitForReload WebDriverWait Object
     */
    private void exportCsvWithMassAction(WebDriverWait waitForReload) {
        traverseToJobTab(waitForReload);
        activeJobsPage.selectAllChkBox().click();
        ...
        popupWindowAfterExportAndVerifyExportedCSV(massoption);
    }

    /**
     * This method is used to close the pop up window display after export as
     * CSV and simultaniously used to get exported file name and compare the
     * file name which is downloaded in drive. @ param WebElement
     * exportedFileName contains exported file name object.
     */
    private void popupWindowAfterExportAndVerifyExportedCSV(
        ...
        for (int i = 0; i < dirContents.length; i++) {
            if (dirContents[i].getName().equals(fileName)) {
                CSLogger.info(
                        "The Exported CSV File Named as " + fileName + "Found");
            } else {
                CSLogger.info("The Exported CSV File Named as " + fileName
                        + "Not Found");
            }
        }
        browserDriver.close();
