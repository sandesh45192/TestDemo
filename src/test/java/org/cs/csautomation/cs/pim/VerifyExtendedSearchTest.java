 ...
 * text attribute which then is assigned to a newly created class and this class
 * is assigned to a product folder which is created along with two child
 * products during runtime and performs advance search operation and also
 * verifies the result
 * 
 * 
 ...
 *
 */
public class VerifyExtendedSearchTest extends AbstractTest {
    ...
    private String                    dataSheetName = "VerifyAdvanceSearch";

    /**
     * This test method performs advance search operation and also verifies the
     * result
     * 
     ...
     * @param searchOption
     *            String object containing search option i.e Advance Search
     * @param searchableTextData
     *            String object containing text data that will be added in some
     *            child product and searched while performing advance search
     * @param operatorOption
     *            String object containing operator option i.e contains,Is Empty
     * @param productTabName
     *            String object containing name of the product tab
     */
    @Test(priority = 1, dataProvider = "advanceSearchTestData")
    public void testVerifyAdvanceSearch(String attributeFolderName,
            ...
            String searchableTextData, String operatorOption,
        ...
        try {
            ...
            String attributeId = createSearchableAttribute(attributeFolderName,
                    textAttributeName, searchOption, textAttributeName);
            ...
            performAdvanceSearchOperation(parentProductName, searchableTextData,
                    childProductId, operatorOption, textAttributeName,
                    attributeId);
            softAssertion.assertAll();
        } catch (Exception e) {
            CSLogger.debug(
                    "Automation error in test method : testVerifyAdvanceSearch",
                    e);
            Assert.fail(
                    "Automation error in test method : testVerifyAdvanceSearch",
                    e);
    ...
    private String createSearchableAttribute(String attributeFolderName,
            String txtAttrName, String searchOption, String attributeName) {
        ...
        selectAdvanceSearchOption(searchOption);
        String attributeId = getAttributeId(txtAttrName);
        return attributeId;
    }

    /**
     * This method selects advance search option for search attribute
     * 
     * @param searchOption
     *            String object containing search option
     */
    private void selectAdvanceSearchOption(String searchOption) {
    ...
    /**
     * Performs advance search operation and also verifies the result
     * 
     * @param productFolderName
     *            String object containing name of parent product folder
     * @param searchableTextData
     ...
     * @param attributeName
     *            String object containing name of attribute
     * @param attributeId
     *            String object containing attribute Id
     */
    private void performAdvanceSearchOperation(String productFolderName,
            String searchableTextData, String childProductId,
            String operatorOption, String attributeName, String attributeId) {
        goToProductsNode();
        clkOnParentProductFolder(productFolderName);
        goToRightSectionWindow();
        ...
        String searchAttribute = attributeName + " " + "(" + attributeId + ")";
        csGuiToolbarHorizontalInstance.clkOnBtnAdvanceSearch(waitForReload);
        csGuiDialogContentIdInstance
                .clkOnDrpDwnInputAttributeSelect(waitForReload);
        csGuiDialogContentIdInstance.selectValueFromDrpDwnElement(
                csGuiDialogContentIdInstance.getDdSearchInputAttributeSelect(),
                searchAttribute);
        csGuiDialogContentIdInstance
                .clkOnDrpDwnInputOperatorSelect(waitForReload);
        csGuiDialogContentIdInstance.selectValueFromDrpDwnElement(
                csGuiDialogContentIdInstance.getDdSearchInputOperatorSelect(),
                operatorOption);
        enterTextInAdvanceSearchTextBox(searchableTextData);
        waitForReload.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//table[@class='CSGuiList ']/tbody/tr[3]/td[contains"
        ...
        verifyAdvanceSearchResult(childProductId, searchableTextData);
    }

    /**
     * Enters text to be searched in advance search text box
     * 
     * @param searchText
     *            String object containing search text
     */
    private void enterTextInAdvanceSearchTextBox(String searchText) {
        CSUtility.waitForVisibilityOfElement(waitForReload,
                csGuiDialogContentIdInstance.getTxtAdvanceSearchInputText());
        csGuiDialogContentIdInstance.getTxtAdvanceSearchInputText().click();
        csGuiDialogContentIdInstance.getTxtAdvanceSearchInputText().clear();
        csGuiDialogContentIdInstance.getTxtAdvanceSearchInputText()
                .sendKeys(searchText);
        CSUtility.tempMethodForThreadSleep(1000);
        csGuiDialogContentIdInstance.getTxtAdvanceSearchInputText()
    ...
    /**
     * Verifies that parent product is found if it contains search text data
     * 
     * @param childProductId
     *            String object containing Id of child product
     * @param searchableTextData
     ...
     */
    private void verifyAdvanceSearchResult(String childProductId,
        ...
        int searchResult = resultTableData
                .findElements(By.xpath("//tr[@id='" + childProductId + "']"))
                .size();
        if (searchResult != 0) {
            CSLogger.info("Product found with search text " + searchableTextData
                    + ": teststep passed");
        } else {
