 ...
 * three child products during runtime and performs distribute content operation
 * on child products and verify the result
 * 
 * 
 * @author CSAutomation Team
 *
 */
public class DistributeProductContentTest extends AbstractTest {
    ...
    private String                    distributeProductContentSheetName = "DistributeProductContent";

    /**
     * This test methods to creates a parent product folder and three child
     * products during runtime and performs distribute content operation on
     ...
     *            name.
     */
    @Test(priority = 1, dataProvider = "distributeProductContentTestData")
    public void testDistributeProductContent(String parentProductName,
        ...
        try {
            ...
            String windowHandle = performDistributeContentOperation();
            verifyDistributeContentWindow(windowHandle, firstChildProductName,
                    secondChildProductName, thirdChildProductName);
            closeDistributeContentWindowTab(windowHandle);
            softAssertion.assertAll();
        } catch (Exception e) {
            CSLogger.debug("Automation Error in testDistributeProductContent",
                    e);
            Assert.fail("Automation Error in testDistributeProductContent", e);
        }
    }
     ...
     *            name.
     */
    @Test(priority = 2, dataProvider = "distributeProductContentTestData")
    public void testDefineNewMasterDistributeContent(String parentProductName,
            String firstChildProductName, String secondChildProductName,
            String thirdChildProductName) {
        try {
            waitForReload = new WebDriverWait(browserDriver, 180);
            selectChildProductsToDistributeContent(parentProductName,
                    firstChildProductName, secondChildProductName,
                    thirdChildProductName);
            String mainWindowHandle = performDistributeContentOperation();
            checkDistributeContentWindow();
            String distributeContentWindowHandle = getDistributeContentWindowHandle();
            ...
            verifyMasterObjectInCheckedInState(parentProductName,
                    firstChildProductName, secondChildProductName,
                    thirdChildProductName, distributeContentWindowHandle);
            defineNewMasterObject(secondChildProductName);
            closeDistributeContentWindowTab(mainWindowHandle);
            softAssertion.assertAll();
        } catch (Exception e) {
            CSLogger.debug(
                    "Automation Error in : testDefineNewMasterDistributeContent",
                    e);
            Assert.fail(
                    "Automation Error in : testDefineNewMasterDistributeContent",
                    e);
    ...
    private void clkOnSelectBoxOfChildProduct(String childProductId) {
        ...
        selectBox.click();
        CSLogger.info(
                "Clicked on selectbox of " + childProductId + "child product");
    }
     ...
     *            name.
     */
    private void verifyDistributeContentWindow(String windowHandle,
    ...
    /**
     * Closes the distribute content window tab and switches back to main window
     * tab
     * 
     * @param windowHandle
     */
    private void closeDistributeContentWindowTab(String windowHandle) {
        browserDriver.close();
        browserDriver.switchTo().window(windowHandle);
        CSLogger.info(
                "Closed distribute content window tab and switches to main "
                        + "window tab");
    }
     ...
     * @param thirdChildProductName String Object containing the child product
     *            name.
     * @param distributeContentWindowHandle String Object containing window
     *            handle of distribute content window
     */
    private void verifyMasterObjectInCheckedInState(String productFolderName,
            String childProductOne, String childProductTwo,
            String childProductThree, String distributeContentWindowHandle) {
        goToPimProductTreeSection();
        clkOnParentProductFolder(productFolderName);
        checkInNonMasterChildProduct(childProductTwo);
        switchToDistributeContentWindowTab(distributeContentWindowHandle);
        clkOnRefreshButton();
    ...
    /**
     * Clicks on refresh button from the distribute content window
     */
     ...
     * 
     * @param newMasterObject String object containing name of the child product
     *            to defined as new master
     */
     ...
     * 
     * @param distributeContentWindowHandle
     */
    private void switchToDistributeContentWindowTab(
            String distributeContentWindowHandle) {
     ...
     * @return Array
     */
    @DataProvider(name = "distributeProductContentTestData")
    public Object[][] getdistributeProductContentData() {
        return DrivenScript.readSheetData(
                config.getExcelSheetPath("pimModuleTestCases"),
                distributeProductContentSheetName);
    }
