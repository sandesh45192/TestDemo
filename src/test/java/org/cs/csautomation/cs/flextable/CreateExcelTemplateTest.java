...
/**
 * This class contains the test methods which verifies Create Excel template
 * test uses.
 * 
 * @author CSAutomation Team
 *
 */
public class CreateExcelTemplateTest extends AbstractTest {
    ...
    private String                    excelTemplateTestData = "ExcelTemplate";

    /**
     * This method verifies Excel template test user.
     * 
     * @param mamFolder String object contains MAM folder name
     * @param filePath String object Contains xls file Path
     ...
     * @param xlsFileName String object contains xls file name
     * @param sheetName String object contains xls file sheet name
     */
    @Test(dataProvider = "ExcelTemplateDataSheet")
    public void testCreateExcelTemplate(String mamFolder, String filePath,
            String className, String flexProduct, String templateName,
            String tempType, String nodeName, String xlsFileName,
            String sheetName) {
        try {
            ...
            addDataInDataPane(mamFolder, templateName, tempType, xlsFileName,
                    nodeName, sheetName);
            ...
            verifyCreatedTable(sheetName);
            addCreatedFlexTable(flexProduct, templateName);
            testResetTableButton();
            verifyAddedFlexTable(sheetName, false);
            addCreatedFlexTable(flexProduct, templateName);
            ...
            CSUtility.tempMethodForThreadSleep(1000);
            checkInProduct();
            verifyAddedFlexTable(sheetName, true);
            softasssert.assertAll();
            CSLogger.info("create excel template test completed");
        } catch (Exception e) {
            CSLogger.debug("Automation Error : testCreateExcelTemplate", e);
            Assert.fail("Automation Error : testCreateExcelTemplate", e);
        }
    }
    ...
    /**
     * This method verifies Excel template test uses.
     * 
     * @param sheetName String object contains Xls file sheet name
     * @param chkStatus boolean object
     */
    private void verifyAddedFlexTable(String sheetName, Boolean chkStatus) {
        try {
     ...
     * 
     * @param worksheetName String object contains Xls file sheet name
     */
    private void verifyCreatedTable(String sheetName) {
        WebElement tableCaption = browserDriver.findElement(
    ...
    private void addDataInDataPane(String mamFolder, String templateName,
            String tempType, String xlsFileName, String nodeName,
            String sheetName) {
    ...
    private void addXmlFile(String mamFolder, String nodeName) {
        ...
        demoNode.click();
        WebElement excelFolder = browserDriver
                .findElement(By.linkText(mamFolder));
        excelFolder.click();
        TraverseSelectionDialogFrames.traverseTillFileFoldersCenterPane(
                waitForReload, browserDriver);
        WebElement elementFile = browserDriver.findElement(By.xpath(
                "//table[@class='hidewrap CSAdminList']/tbody/tr[3]/td[2]"));
        CSUtility.waitForVisibilityOfElement(waitForReload, elementFile);
        action.doubleClick(elementFile).build().perform();
    }
     ...
     * TemplateName,TemplateType,NameOfNode,NameXslFile,Sheet Name
     * 
     * @return excelTemplateTestData
     */
    @DataProvider(name = "ExcelTemplateDataSheet")
    public Object[] ExcelTemplate() {
        return DrivenScript.readSheetData(
                config.getExcelSheetPath("flexTableTestCases"),
                excelTemplateTestData);
    }
