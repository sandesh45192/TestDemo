...
/**
 * This class verifies registration with existing mail and password
 * 
 * @author CSAutomation Team
 *
 */
public class VerifyExistingMailAndPasswordTest extends AbstractTest {
    ...
    private String           verifyExistingMailAndPasswordSheet = "VerifyExistingMailAndPassword";

    /**
     * This method verifies existing mail and password registration
     * 
     * @param mailAddress contains mail address
     * @param password contains password
     */
    @Test(priority = 1, dataProvider = "verifyExistingMailAndPassword")
    public void testVerifyExistingMailAndPassword(String mailAddress,
            String password) {
        try {
            loginPage.clkSignUp(waitForReload);
            registerWithExistingMail(mailAddress, password);
            verifyAlertMessage();
        } catch (Exception e) {
    ...
    private void verifyAlertMessage() {
        try {
            ...
            if (alertText.equals("User name is no longer available.")) {
                alert.accept();
     ...
     * @param password contains password
     */
    private void registerWithExistingMail(String emailAddress,
            String password) {
     ...
     * @return
     */
    @DataProvider(name = "verifyExistingMailAndPassword")
    public Object[] verifyEmail() {
        return DrivenScript.readSheetData(
                config.getExcelSheetPath("userManagementTestCases"),
                verifyExistingMailAndPasswordSheet);
    }
